angular
		...
		.controller(
				...
				function($http, $scope, $rootScope, clientService) {
					...
					$scope.getIncomes = function(incomesMode) {
						// $scope.myCoupons = "";
						var url = null;
						if (incomesMode == null || incomesMode == ""
								|| incomesMode == "ALL") {
							url = clientService.restURL + "admin/income";
//						} else if (incomesMode == "BY_ID") {
//							url = clientService.restURL + "admin/income/"
//									+ $scope.companyId;
						}
						console.log("getting incomes from : " + url);
						$http
								.get(url)
								.then(
										function(response) {
											...
											if (angular.isArray(response.data)) {
												...
												console
														.log("got single income, clearing the old incomes array, and get response to incomes[0]");
												$scope.incomes = [];
												$scope.incomes[0] = response.data;
											}
										});
					}
		...
		.controller(
				...
				function($http, $scope, $rootScope, clientService) {
					...
					// show companies ( all / by id )
					$scope.income = function(incomeMode) {
						// $scope.myCoupons = "";
						var url = null;
						if (incomeMode == null || incomeMode == ""
								|| incomeMode == "ALL") {
							url = clientService.restURL + "admin/income";
//						} else if (incomesMode == "BY_ID") {
//							url = clientService.restURL + "admin/income/"
//									+ $scope.companyId;
						...
						console.log("getting income from : " + url);
						$http
								.get(url)
								.then(
										function(response) {
